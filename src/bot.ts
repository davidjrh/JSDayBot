...
import { ActivityTypes, CardFactory, ConversationState, RecognizerResult, StatePropertyAccessor, TurnContext, UserState, ActionTypes } from 'botbuilder';
import { DialogSet, DialogContext, DialogState, DialogTurnResult, DialogTurnStatus, ChoicePrompt, Dialog, ComponentDialog, WaterfallDialog } from 'botbuilder-dialogs';
import { QnAMaker, QnAMakerResult, LuisRecognizer } from 'botbuilder-ai';

import { WelcomeCard } from './dialogs/welcome';
import { UserProfile, GreetingDialog } from './dialogs/greeting';
import { BotConfiguration, LuisService, QnaMakerService } from 'botframework-config';
import { getTime, getSessionDetails } from './functions';
import { SpeakerSession } from "./types";
import { getData } from "./parser";
import { createCarousel, createHeroCard } from "./cards";
import { BotAnalytics } from "./analytics";
...
export class JSDayBot {
  ...
  constructor(conversationState: ConversationState, userState: UserState, botConfig: BotConfiguration) {
    ...
    // add the QnA maker 
    let qnaMakerConfig: QnaMakerService;
    qnaMakerConfig = <QnaMakerService>botConfig.findServiceByNameOrId(QNA_CONFIGURATION);
    if (!qnaMakerConfig || !qnaMakerConfig.kbId) throw ('Missing QnA configuration. Please follow README.MD to create required QnA applications.\n\n');
    ...
    this.botAnalytics = new BotAnalytics();
  }
        ...
        switch (dialogResult.status) {
          ...
          case DialogTurnStatus.empty:
            ...
            switch (topIntent) {
              ...
              case TIME_INTENT:
                await dc.context.sendActivities(getTime(data));        
              ...
              case LOCATION_INTENT:
                if(data.length > 1) {
                  await context.sendActivity(createCarousel(data, topIntent));
                  this.botAnalytics.logIntent(topIntent, dc, results, false);
                }
                else if (data.length === 1) {
                  await context.sendActivity({ attachments: [createHeroCard(data[0], topIntent)] });
                  this.botAnalytics.logIntent(topIntent, dc, results, false);
                }
                else {
                  await context.sendActivity("No tengo ninguna sesión en la agenda para eso. Prueba a preguntarme por el nombre del speaker o de lo que se va a hablar en la misma.");
                  this.botAnalytics.logIntent(topIntent, dc, results, true);
                }
                break;

              case SESSIONDETAILS_INTENT:
                await dc.context.sendActivities(getSessionDetails(data));        
                this.botAnalytics.logIntent(topIntent, dc, results, false);
                break;
              ...
              default:
                ...
                const qnaResults: QnAMakerResult[] = await this.qnaMaker.generateAnswer(context.activity.text);
                ...
                if (qnaResults[0]) {
                  ...
                  // to the user
                  await dc.context.sendActivity(`Lo siento, no entendí lo que quisiste decirme.`);
                  this.botAnalytics.logIntent(topIntent, dc, results, true);
                }              
                break;
            }
        ...
        for (var idx in context.activity.membersAdded) {
          ...
          if (context.activity.membersAdded[idx].id !== context.activity.recipient.id) {
            ...
            const welcomeCard = CardFactory.heroCard(
              "¡Bienvenido al JSDay Canarias 2018!",
  ...
  private isTurnInterrupted = async (dc: DialogContext, luisResults: RecognizerResult) => {
    ...
    if (topIntent === CANCEL_INTENT) {
      if (dc.activeDialog) {
        ...
        await dc.cancelAllDialogs();
        await dc.context.sendActivity(`Vale, he cancelado nuestra última actividad.`);
      } else {
        await dc.context.sendActivity(`No tengo nada que cancelar.`);
      }
