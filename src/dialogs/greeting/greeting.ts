...
export class GreetingDialog extends ComponentDialog {
  ...
  private promptForNameStep = async (step: WaterfallStepContext<UserProfile>) => {
    ...
    if (!userProfile.name) {
      ...
      return await step.prompt(NAME_PROMPT, 'Hola, soy el bot del JS Day Canarias. ¿Cómo te llamas?');
    } else {
      return await step.next();
    }
  }
  ...
  private promptForCityStep = async (step: WaterfallStepContext<UserProfile>) => {
    ...
    if (!userProfile.city) {
      ...
      return await step.prompt(CITY_PROMPT, `Hola ${userProfile.name}, ¿en qué ciudad vives?`);
    } else {
      return await step.next();
    }
  }
  ...
  private validateName = async (validatorContext: PromptValidatorContext<String>) => {
    ...
    if (value.length >= NAME_LENGTH_MIN) {
      ...
      await validatorContext.context.sendActivity(`El nombre debe tener al menos ${NAME_LENGTH_MIN} caracteres.`);
  ...
  private validateCity = async (validatorContext: PromptValidatorContext<String>) => {
    ...
    if (value.length >= CITY_LENGTH_MIN) {
      ...
      await validatorContext.context.sendActivity(`El nombre de la ciudad tiene que tener al menos ${CITY_LENGTH_MIN} caracteres.`);
      return VALIDATION_FAILED;
    }
  }
  /**
  ...
  async greetUser(step: WaterfallStepContext) {
    ...
    await step.context.sendActivity(`Hola ${userProfile.name}, de ${userProfile.city}, ¡encantado de saludarte!`);
    await step.context.sendActivity(`Siempre puedes decir 'Mi nombre es <tu nombre> para volver a presentarte.`);
